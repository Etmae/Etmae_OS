...
export const Terminal = () => {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#0c0c0ce6] backdrop-blur-xl text-[#cccccc] font-['Cascadia_Code',_monospace] text-sm overflow-hidden shadow-2xl">
      
      {/* Tab Bar (Windows 11 Style) */}
      <div className="flex items-center bg-[#1e1e1e] px-2 pt-1 h-10 select-none">
        {tabs.map(tab => (
          <div
            key={tab.id}
            onClick={() => setActiveTabId(tab.id)}
            className={`group relative flex items-center h-full px-4 min-w-[120px] max-w-[200px] rounded-t-lg transition-colors cursor-default border-b-2 ${
              activeTabId === tab.id 
