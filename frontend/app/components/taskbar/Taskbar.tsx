...
const TaskbarApp: React.FC<TaskbarAppProps> = ({ 
    ...
    >
      <div
        className={`transition-transform duration-200 ${
          isFocused ? 'scale-90' : 'scale-100 group-active:scale-75'
        }`}
      >
        {renderIcon()}
      </div>
...
const WindowsLogo: React.FC<{ active: boolean }> = ({ active }) => (
  <div className="w-5 h-5 grid grid-cols-2 gap-0.5">
    {[1, 2, 3, 4].map((i) => (
      <div
        key={i}
        className={`rounded-sm transition-colors ${active ? 'bg-[#00a3ee]' : 'bg-[#0078d4]'}`}
      />
    ))}
  </div>
);
...
const WeatherWidget: React.FC<{
  ...
  onHoverEnter: () => void;
  onHoverLeave: () => void;
}> = ({ isOpen, onToggle, onHoverEnter, onHoverLeave }) => (
...
const Taskbar: React.FC<TaskbarProps> = ({ onLock, onRestart, onShutdown }) => {
  ...
  const handlePowerAction = (type: 'shutdown' | 'restart') => {
    ...
    setTimeout(() => {
      type === 'shutdown' ? onShutdown?.() : onRestart?.();
    }, 4000);
  };
        ...
        <div className="relative w-full h-full flex items-center justify-center px-2">
          ...
          {!isMobile && (
            <WeatherWidget
              ...
              onHoverEnter={handleWidgetsEnter}
              onHoverLeave={handleWidgetsLeave}
            />
          )}
          ...
          <div className="absolute right-2 h-full flex items-center gap-1">
            <button
              ...
              className="p-2 rounded-md hover:bg-white/10 text-white/80 transition-colors flex"
            ...
            {!isMobile && (
              <>
                <SystemTray
                  ...
                  onToggle={() => setQuickSettingsOpen((v) => !v)}
                />
                <DateTimeDisplay
