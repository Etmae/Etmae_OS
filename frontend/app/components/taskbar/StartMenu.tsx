...
interface StartMenuProps {
  isOpen: boolean;
  onClose: () => void;
  desktopIcons: Array<{ icon: React.ReactNode; label: string; appId: string }>; // Changed route to appId
  recentFiles: Array<{ name: string; time: string; icon: React.ReactNode }>;
  powerMenuOpen: boolean;
  setPowerMenuOpen: (open: boolean) => void;
  handleLock: () => void;
  handleRestart: () => void;
  handleShutdown: () => void;
  onAppClick: (appId: string) => void; // Added this prop
}
      ...
      {isOpen && (
        <>
          ...
          <motion.div 
            ...
            transition={{ type: "spring", stiffness: 500, damping: 35, mass: 0.7 }}
          >
            <div className={`rounded-2xl overflow-hidden ${isMobile ? 'mx-3 mb-3' : ''}`} style={{ background: 'rgba(30, 30, 30, 0.95)', backdropFilter: 'blur(40px) saturate(180%)', border: '1px solid rgba(255, 255, 255, 0.12)', borderRadius: '16px', boxShadow: '0 8px 32px rgba(0, 0, 0, 0.5)' }}>
              
              {/* Search Bar */}
              <div className="p-4 sm:p-6 pb-4 sm:pb-5">
                <div className="rounded-lg px-5 py-3 flex items-center gap-3" style={{ background: 'rgba(255, 255, 255, 0.08)', border: '1px solid rgba(255, 255, 255, 0.05)' }}>
                  <Search size={18} className="text-white/50" />
                  <input type="text" placeholder="Search for apps, settings, and documents" className="flex-1 bg-transparent text-white placeholder-white/40 outline-none text-sm" />
                </div>
              </div>
              ...
              <div className="px-4 sm:px-6 pb-4 sm:pb-5">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-white/70 text-xs font-semibold uppercase tracking-wider">Pinned</span>
                  <button className="text-white/50 hover:text-white text-xs transition-colors">All &gt;</button>
                </div>
                <div className={`grid gap-3 ${isMobile ? 'grid-cols-4' : 'grid-cols-6'}`}>
                  {desktopIcons.map((item, index) => (
                    <button
                      ...
                      className="flex flex-col items-center gap-2 p-3 rounded-lg transition-all duration-200 hover:bg-white/10 active:scale-95"
                      onClick={() => {
                        onAppClick(item.appId); // Now opens the app
                        onClose();
                      }}
                    >
                      <div className="text-white">
                      ...
                      <span className="text-white/85 text-[11px] text-center leading-tight truncate w-full">{item.label}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              {/* Recommended Section */}
              <div className="px-4 sm:px-6 pb-4 sm:pb-5 border-t border-white/10 pt-4 sm:pt-5 max-h-[40vh] overflow-y-auto">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-white/70 text-xs font-semibold uppercase tracking-wider">Recommended</span>
                  <button className="text-white/50 hover:text-white text-xs transition-colors">More &gt;</button>
                </div>
                <div className="space-y-1">
                  {recentFiles.map((file, index) => (
                    <button key={index} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200 hover:bg-white/10 text-left">
                      <div className="shrink-0 text-white/70">{file.icon}</div>
                      <div className="flex-1 min-w-0">
                        <div className="text-white text-sm font-medium truncate">{file.name}</div>
                        <div className="text-white/50 text-xs">{file.time}</div>
                      </div>
                    </button>
                  ))}
                </div>
              ...
              {/* User Profile & Power */}
              <div className="px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between" style={{ background: 'rgba(20, 20, 20, 0.7)', borderTop: '1px solid rgba(255, 255, 255, 0.08)' }}>
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full flex items-center justify-center bg-blue-600"><User size={16} className="text-white" /></div>
                  <span className="text-white text-sm font-medium">Elijah Olujimi</span>
                </div>
                <div className="relative">
                  <button onClick={(e) => { e.stopPropagation(); setPowerMenuOpen(!powerMenuOpen); }} className="p-2 rounded-lg hover:bg-white/10">
                    <Power size={18} className="text-white/80" />
                  </button>
                  <PowerMenu isOpen={powerMenuOpen} onClose={() => setPowerMenuOpen(false)} onLock={handleLock} onRestart={handleRestart} onShutdown={handleShutdown} />
                </div>
              </div>
            </div>
          </motion.div>
