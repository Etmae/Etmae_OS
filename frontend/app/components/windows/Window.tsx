...
export const Window = memo(({ id }: WindowProps) => {
  ...
  useEffect(() => {
    ...
    console.log('Starting snapshot capture for window', id, 'minimized:', win?.isMinimized, 'has snapshot:', !!win?.snapshot);
    ...
    const normalizeCssString = (property: string, value: string) => {
      if (!value || !/(oklab|lab|lch|color\(|hwb\()/i.test(value)) return value;
      return value.replace(/(oklab\([^)]*\)|lab\([^)]*\)|lch\([^)]*\)|color\([^)]*\)|hwb\([^)]*\))/gi, (match) => {
        return normalizeCssValue(property, match);
      });
    };
    ...
    const sanitizeClonedStyles = (source: HTMLElement, cloned: HTMLElement) => {
      ...
      while (queue.length) {
        ...
        for (let i = 0; i < computed.length; i += 1) {
          ...
          if (!value) continue;
          const normalized = normalizeCssString(prop, value);
          if (normalized !== value) {
    ...
    const captureSnapshot = async () => {
      // Selector targets the client area div
      const node = document.querySelector(
        `[data-window-id="${id}"] [data-window-content]`
      ) as HTMLElement | null;
      if (!node) {
        console.warn('Snapshot target not found for window', id);
        return;
      }
      console.log('Found snapshot target for window', id, node);
      ...
      try {
        ...
        if (dataUrl) {
          console.log('Captured snapshot for window', id, 'length:', dataUrl.length);
          updateWindowSnapshot(id, dataUrl);
        } else {
          console.warn('html2canvas returned empty dataUrl for window', id);
        }
      } catch (error) {
        console.error('Failed to capture window snapshot:', error);
        // Gracefully degrade on snapshot capture failure; the taskbar thumbnail will display a placeholder icon.
      }
    };

    const timeout = setTimeout(captureSnapshot, 1000);
